...
def rebalance_routes(res, min_clients=10, max_clients=25):
    ...
    respeitando capacidades dos veículos ao reatribuir.
    """
    # Identifica rotas fora do intervalo
    rotas_ruins = [i for i, r in enumerate(res["routes"]) if len(r) - 2 < min_clients or len(r) - 2 > max_clients]
    if not rotas_ruins:
        return res  # nada a fazer

    # Coleta pedidos dessas rotas
    pedidos_rebalancear_rows = []
    for i in rotas_ruins:
        for node in res["routes"][i][1:-1]:
            pedidos_rebalancear_rows.append(res["orders"].iloc[node - 1])
    pedidos_rebalancear = pd.DataFrame(pedidos_rebalancear_rows)

    if pedidos_rebalancear.empty:
        return res

    # Mantém rotas boas
    rotas_boas = [r for i, r in enumerate(res["routes"]) if i not in rotas_ruins]

    # Reotimiza apenas os pedidos a rebalancear
    ...
    # Junta rotas boas + novas rotas rebalanceadas
    rotas_final = rotas_boas + novas_rotas
    cargo_final = ([res["cargo"][i] for i in range(len(res["routes"])) if i not in rotas_ruins] + novas_cargas)

    # Reatribui veículos para o conjunto final
    assignments_final, dists_final = assign_vehicles_to_routes(rotas_final, cargo_final, FLEET, dist_km_rb)
    ...
    return {
        ...
        "nodes": nodes_rb,
        "dist_km": dist_km_rb,
        "violations": res["violations"],  # mantém as violações calculadas previamente (opcional recalcular)
        "orders": res["orders"],          # mantém o mesmo dataframe de pedidos do dia
    }
