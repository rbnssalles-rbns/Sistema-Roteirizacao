...
def estimate_volume_weight(sku_qty: Dict[str,int]):
    """Estimativa de volume e peso usando o CATALOG."""
    vol, wt = 0, 0
...
def try_append_to_existing_route(date, client_id, sku_qty, res):
    """Verifica se o cliente já está em alguma rota e se há capacidade para incluir o pedido extra sem reotimização."""
...
def _map_center_from_orders(res):
    """Calcula centro do mapa a partir das coordenadas dos pedidos do dia."""
    ...
    return (-3.732, -38.526)  # fallback: Fortaleza

def plot_routes_map(res, depot_coords=None, zoom_start=11):
    """
    Plota rotas em mapa interativo com Folium.
    Cada rota recebe uma cor diferente.
    """
...
# =====================================
# Estado da sessão: pedidos extras
# =====================================
...
# =====================================
# Interface de planejamento D+1 + capacidade
# =====================================
...
if st.button("Planejar dia seguinte (D+1)", key="btn_plan_next"):
    ...
    if idx >= len(DATES)-1:
        ...
        # Checkbox para mostrar mapa do D+1
        mostrar_mapa_d1 = st.checkbox("Mostrar mapas das rotas (D+1)", value=False, key="chk_map_d1")
        if mostrar_mapa_d1:
            DEPOT_COORDS = st.session_state.get("depot_coords", None)
            plot_routes_map(res_next, depot_coords=DEPOT_COORDS)
...
# Inputs dinâmicos de SKUs a partir de CATALOG
sku_inputs = {}
...
if st.button("Adicionar pedido extra e replanejar", key="btn_pedido_extra"):
    ...
    # Checkbox para mostrar mapa após pedido extra
    mostrar_mapa_extra = st.checkbox("Mostrar mapas das rotas (após pedido extra)", value=False, key="chk_map_extra")
    if mostrar_mapa_extra:
        DEPOT_COORDS = st.session_state.get("depot_coords", None)
        plot_routes_map(res_mod, depot_coords=DEPOT_COORDS)

    # Exportação do cenário modificado (CSV)
...
if st.button("Simular semana completa", key="btn_semana"):
    DEPOT_COORDS = st.session_state.get("depot_coords", None)  # opcional
    for d in DATES:
        df_mod = DAILY.copy()
        for pedido in st.session_state.pedidos_extras:
        ...
        daily_summary(res_dia)
        # Gráficos estáticos por veículo (se já usa)
        for i in range(len(res_dia["routes"])):
            plot_routes_by_vehicle(res_dia, i, figsize=(6,6))
        # Checkbox para mostrar mapa por dia
        mostrar_mapa_semana = st.checkbox(f"Mostrar mapa de {pd.to_datetime(d).date()}", value=False, key=f"chk_map_{pd.to_datetime(d).date()}")
        if mostrar_mapa_semana:
