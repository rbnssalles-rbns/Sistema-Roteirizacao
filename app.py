...
## 6. Execução por dia e relatório
#Parte A — Funções de negócio e rebalanceamento revisado
...
def daily_summary(result):
    ...
    if result.get("anomalies"):
        st.warning(f"⚠️ Rotas anômalas (menos de 10 clientes): {', '.join(result['anomalies'])}")
    if result.get("unrouted_clients"):
        st.warning(f"⚠️ Clientes originalmente não agrupáveis foram alocados em rotas anômalas: {len(result['unrouted_clients'])}")
...
# -----------------------------
# Função de rebalanceamento revisada
# -----------------------------

def rebalance_routes(res, min_clients=10, max_clients=25):
    """
    Rebalanceia rotas para ter entre min_clients e max_clients paradas.
    - Divide rotas grandes (> max_clients) em blocos menores
    - Agrupa rotas pequenas para tentar formar rotas com >= min_clients
    - Se ainda restarem clientes dispersos, cria rotas anômalas (< min_clients) para garantir entrega
    - Reatribui veículos e recalcula métricas e violações
    """
    ...
    # Violações de janelas (usa matriz do dia)
    violations = []
...
#Parte B — Interface Streamlit e pedidos extras preservando rotas
# -----------------------------
# Interface Streamlit
# -----------------------------
...
# Guardar última versão reprocessada como base oficial
if "res_base" not in st.session_state:
    st.session_state.res_base = st.session_state.res
...
# Filtro de rotas (multiselect único)
...
# Botão de reprocessar com rebalanceamento
if st.button("🔄 Reprocessar rotas", key="btn_reprocessar"):
    ...
    if st.checkbox("Mostrar mapas das rotas (após reprocessar)", key="mostrar_mapas_checkbox_reprocessar"):
        for i in range(len(res["routes"])):
            if f"Rota {i}" in st.session_state.rotas_selecionadas:
                with st.container():
                    plot_routes_by_vehicle(res, i, figsize=(7, 7))
...
# -----------------------------
# Pedidos extras preservando rotas
# -----------------------------
...
def estimate_volume_weight(sku_qty: dict):
    ...
    # Ajuste de empilhamento como no enrich_orders
...
def adicionar_pedido_extra_e_replanejar(date_sel: pd.Timestamp, client_id: str, sku_qty: dict):
    ...
    res_mod = simulate_day(date_sel, df_mod)

    if rota_idx is None:
        # Cliente não estava em rota: rebalanceia e sinaliza
        st.warning(f"Cliente {client_id} não estava em rotas base. Rebalanceando para incluir.")
        res_mod = rebalance_routes(res_mod, min_clients=10, max_clients=25)
        st.session_state.res = res_mod
        return res_day_original, res_mod

    # Cliente estava em rota: tenta manter estrutura e só atualizar carga/métricas
    ...
    if assign_v and (vol_atual + vol_extra <= assign_v.vol_m3) and (wt_atual + wt_extra <= assign_v.weight_kg):
        # Atualiza carga na rota e mantém estrutura
        res_base["cargo"][rota_idx] = (vol_atual + vol_extra, wt_atual + wt_extra, svc_atual + svc_extra)

        # Recalcula violações na rota afetada usando matriz do dia com DF modificado
        res_matriz = simulate_day(date_sel, df_mod)  # para obter dist_km/time_h e orders atualizados
        ...
        for node in rota[1:-1]:
            ...
            travel_h = res_matriz["dist_km"][prev, node]
            t += dt.timedelta(hours=travel_h)
            ord_row = res_matriz["orders"].iloc[node - 1]
            if not (ord_row["tw_start"] <= t.time() <= ord_row["tw_end"]):
                violacoes.append({
                    "route": rota_idx,
                    "client_id": ord_row["client_id"],
        ...
        st.success(f"Pedido extra do cliente {client_id} alocado na mesma rota {rota_idx} sem desagrupar.")
        if violacoes:
            st.warning(f"⚠️ {len(violacoes)} violações de janela após adicionar pedido extra.")
        return res_day_original, res_base

    # Se não coube no veículo atual, rebalanceia para incluir e reatribuir
    st.warning(f"Capacidade insuficiente na rota {rota_idx}. Rebalanceando para incluir pedido extra.")
    res_mod = rebalance_routes(res_mod, min_clients=10, max_clients=25)
    st.session_state.res = res_mod
...
st.subheader("📆 Planejamento Rolling (D+1)")
...
if st.button("Adicionar pedido extra e replanejar", key="btn_pedido_extra"):
    if not sku_inputs:
        ...
        before_res, after_res = adicionar_pedido_extra_e_replanejar(pd.to_datetime(data_extra), cliente_extra, sku_inputs)

        st.subheader("📊 Comparação antes/depois")
        col1, col2 = st.columns(2)
        with col1:
            st.caption("Antes do pedido extra")
            daily_summary(before_res)
        with col2:
            st.caption("Depois do pedido extra")
            daily_summary(after_res)

        for i in range(len(after_res["routes"])):
            plot_routes_by_vehicle(after_res, i, figsize=(7,7))

# Tabela de pedidos extras acumulados
if "pedidos_extras" in st.session_state and st.session_state.pedidos_extras:
...
# Planejamento rolling automatizado com extras
st.subheader("📈 Simular toda a semana com pedidos extras")
if st.button("Simular semana completa", key="btn_semana"):
    for d in DATES:
        df_mod = DAILY.copy()
        for pedido in (st.session_state.pedidos_extras or []):
            if pd.to_datetime(pedido.get("date")) == d:
                df_mod = add_last_minute_order(d, pedido["client_id"], pedido["sku_qty"], df_mod)
