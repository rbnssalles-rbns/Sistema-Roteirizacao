...
if st.button("Simular semana completa com extras", key="btn_semana_completa_extras"):
    ...
    for d in DATES:
        ...
        # Proteção contra erro de assign ausente
        assigns = res_dia.get("assign", [])
        veiculos_usados = len(set(v for _, v in assigns)) if assigns else 0

        resumo_semana.append({
            "Data": d.date(),
            "Rotas": len(res_dia["routes"]),
            "Veículos": veiculos_usados,
            "Distância total (km)": sum(r[2] for r in res_dia["routes"]),
            "Violações": len(res_dia["violations"]),
        })

        st.markdown(f"### 📅 {d.date()}")
        daily_summary(res_dia)

        if len(res_dia["violations"]) > 15:
