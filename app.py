...
def simulate_day(date: pd.Timestamp, daily_df: pd.DataFrame):
    ...
    # Simulação simples de janelas (partindo do WORK_START)
...
# Parte B — Interface Streamlit + pedidos extras (consolidada)
...
if st.button("🔄 Reprocessar rotas", key="btn_reprocessar_principal"):
    ...
    rotas_disponiveis = [f"Rota {i}" for i in range(len(res["routes"]))]  # CORRIGIDO: removido parêntese extra
    st.session_state.rotas_selecionadas = [
