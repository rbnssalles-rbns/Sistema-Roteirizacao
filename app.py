...
if st.button("Planejar dia seguinte (D+1)", key="btn_plan_next_rolling"):
    ...
    if idx >= len(DATES) - 1:
        ...
        # Simula com base atualizada (respeitando extras)
...
# Interface de pedidos extras (chaves únicas)
...
if st.button("Adicionar pedido extra e replanejar", key="btn_pedido_extra_exec"):
    if not sku_inputs:
        ...
        st.success(f"✅ Pedido extra do cliente {cliente_extra} registrado e replanejado com sucesso!")

        # Limpa campos para novo pedido
        st.session_state["inp_data_extra"] = min(DATES)
        # Define primeiro cliente como padrão (ou mantenha último, se preferir)
        st.session_state["sel_cliente_extra"] = CLIENTS["client_id"].unique()[0]
        for sku in CATALOG["sku"].unique():
            st.session_state[f"inp_qtd_{sku}_extra"] = 0
...
# Histórico de pedidos extras + download
if st.session_state.pedidos_extras:
    ...
    # Melhor apresentação: explode sku_qty em colunas (opcional)
    # Mantemos simples para não travar a interface com muitos SKUs
