...
def estimate_volume_weight(sku_qty: Dict[str,int]):
    """Estimativa de volume (m³) e peso (kg) usando o CATALOG."""
    vol, wt = 0, 0
...
def add_last_minute_order(date: pd.Timestamp, client_id: str, sku_qty: Dict[str,int], daily_df: pd.DataFrame):
    """Adiciona pedido extra ao DF diário e enriquece com CLIENTS."""
...
def try_append_to_existing_route(date, client_id, sku_qty, res):
    """Inclui extra no mesmo cliente da rota se houver capacidade, sem reotimizar."""
...
def _map_center_from_orders(res):
    """Centro do mapa via média das coordenadas dos pedidos."""
...
def plot_routes_map(res, depot_coords=None, zoom_start=11):
    """Mapa Folium das rotas, uma cor por rota, com marcadores dos clientes."""
...
def validate_min_clients(res, minimo=20, title_prefix=""):
    """Valida mínimo de clientes por rota e sinaliza exceções com impacto financeiro."""
    st.subheader(f"✅ Validação de mínimo de clientes {title_prefix}".strip())
    for i, route in enumerate(res["routes"]):
        # Conta apenas clientes (exclui nós 0 de depósito)
...
# Botão: adicionar pedido extra e replanejar
