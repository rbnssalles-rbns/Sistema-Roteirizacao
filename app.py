...
st.header("📆 Planejamento Rolling (D+1)")
...
# Interface para escolher a data corrente
...
if st.button("Planejar dia seguinte (D+1)", key="btn_plan_next"):
    res_next = plan_next_day(data_corrente, DAILY)
    if res_next:
...
qtd_pet = st.number_input("PET 2L", min_value=0, step=1, key="qtd_pet")
qtd_caixa = st.number_input("Caixa 12x1L", min_value=0, step=1, key="qtd_caixa")
qtd_bombona = st.number_input("Bombona 20L", min_value=0, step=1, key="qtd_bombona")

if st.button("Adicionar pedido extra e replanejar", key="btn_pedido_extra"):
    DAILY_MOD = add_last_minute_order(
        pd.to_datetime(data_extra),
        cliente_extra,
        {"PET_2L": qtd_pet, "Caixa_12x1L": qtd_caixa, "Bombona_20L": qtd_bombona},
        DAILY
    )
    res_mod = simulate_day(pd.to_datetime(data_extra), DAILY_MOD)
