...
# ... suas outras imports/definições: DEPOT, FLEET, DATES, DAILY, WORK_START, SERVICE_MINUTES,
# ... e funções: build_costs_for_day, clarke_wright, assign_vehicles_to_routes
...
def daily_summary(result):
    ...
    for i, r in enumerate(result["routes"]):
        ...
        dist = result["dists"][i]
        stops = len(r) - 2
        st.write(f"- Rota {i}: veículo={vname}, paradas={stops}, dist={dist:.1f} km, carga={vol:.2f} m³ / {wt:.0f} kg")
...
def plot_routes_by_vehicle(result, figsize=(7,7)):
    ...
    for i, r in enumerate(result["routes"]):
        ...
        ax.set_title(f"Rota {i} – Veículo: {vname} ({vvol} m³ / {vwt} kg) – {result['date'].date()}")
...
# 1. Menu suspenso para escolher o dia
dia_selecionado = st.selectbox("Selecione o dia", DATES)

# 2. Botão para reprocessar rotas
if st.button("🔄 Reprocessar rotas"):
    res_mod = simulate_day(dia_selecionado, DAILY)
    st.success("Rotas reprocessadas com sucesso!")
    daily_summary(res_mod)
    # 3. Checkbox para mostrar mapas
    if st.checkbox("Mostrar mapas das rotas"):
        plot_routes_by_vehicle(res_mod)
else:
    # Execução padrão
    res_mod = simulate_day(dia_selecionado, DAILY)
    daily_summary(res_mod)
