...
if st.button("Simular semana completa com extras", key="btn_semana_completa_extras"):
    ...
    for d in DATES:
        ...
        # Proteção contra assign malformado
        assigns = res_dia.get("assign", [])
        veiculos_usados = len(set(v[1] for v in assigns if isinstance(v, (list, tuple)) and len(v) > 1))

        # Coleta métricas para resumo semanal
    ...
    # Resumo consolidado da semana (fora do loop, alinhado ao nível do for)
    df_resumo = pd.DataFrame(resumo_semana)
...
def build_folium_map(res_dia):
    """
    Gera um mapa Folium com as rotas do dia.
    Cada rota recebe uma cor diferente e os clientes são marcados.
    Rotas anômalas (ex: violações) aparecem em vermelho.
    """
    orders = res_dia["orders"]

    # Inicializa mapa centralizado no primeiro cliente
    if not orders.empty:
        lat0, lon0 = orders.iloc[0]["lat"], orders.iloc[0]["lon"]
        mapa = folium.Map(location=[lat0, lon0], zoom_start=11)
    else:
        mapa = folium.Map(location=[-3.717, -38.543], zoom_start=11)  # fallback Fortaleza

    cores = ["blue", "green", "purple", "orange", "cadetblue", "darkred"]

    for idx, rota in enumerate(res_dia["routes"]):
        cor = cores[idx % len(cores)]

        # Se rota tiver violações → marca como anômala
        violacoes_rota = [v for v in res_dia["violations"] if v["route"] == idx]
        if violacoes_rota:
            cor = "red"

        pontos = []
        for node in rota:
            ord_row = orders.iloc[node - 1] if node > 0 else None
