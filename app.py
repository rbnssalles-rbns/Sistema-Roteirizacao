...
# Parte A — Funções de negócio + rebalanceamento revisado
# (Pressupõe que DATES, DAILY, FLEET, DEPOT, CLIENTS, CATALOG,
#  SERVICE_MINUTES, WORK_START, STACKING_EFFICIENCY,
#  build_costs_for_day, clarke_wright, assign_vehicles_to_routes
#  já estejam definidos nas seções anteriores do notebook.)
...
def rebalance_routes(res, min_clients=10, max_clients=25):
    """
    Rebalanceia rotas para obedecer min/max de clientes:
    - Divide rotas grandes (> max_clients) em blocos
    - Reagrupa clientes de rotas pequenas (< min_clients)
    - Se ainda restarem, cria rotas anômalas (< min_clients) e sinaliza
    - Reatribui veículos e recalcula violações
    """
    rotas_final = []
    cargo_final = []
    dispersos_rows = []
...
#Parte B — Interface Streamlit + botão de pedidos extras
# Parte B — Interface Streamlit + pedidos extras
# (Pressupõe Parte A carregada e seções anteriores definidas)
...
# Filtro de rotas (multiselect — chave única)
...
# Botão de reprocessar com rebalanceamento (chave única)
...
# =========================
# Planejamento Rolling (D+1) + Pedidos Extras
# =========================
st.header("📆 Planejamento Rolling (D+1) + Pedidos Extras")

# Helpers de pedidos extras
...
def adicionar_pedido_extra_e_replanejar(date_sel: pd.Timestamp, client_id: str, sku_qty: dict):
    ...
    # Encontra rota do cliente na base
    rota_idx = None
    ...
    # Atualiza DF com pedido extra e recalcula matriz
    ...
    if assign_v and (vol_atual + vol_extra <= assign_v.vol_m3) and (wt_atual + wt_extra <= assign_v.weight_kg):
        ...
        # Recalcula violações para a rota afetada
        ...
        for node in rota[1:-1]:
            ...
            travel_h = res_mod_matriz["dist_km"][prev, node]
            t += dt.timedelta(hours=travel_h)
...
# Planejamento D+1 (selectbox e botão com chaves únicas)
...
# Interface de pedidos extras (todas as chaves únicas)
...
# Simular semana completa (botão com chave única)
st.subheader("📈 Simular toda a semana com pedidos extras")
if st.button("Simular semana completa", key="btn_semana_completa"):
    for d in DATES:
        df_mod = DAILY.copy()
        # Se desejar armazenar extras, use st.session_state.pedidos_extras com chaves exclusivas
