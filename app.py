...
rotas_selecionadas = st.multiselect("Escolha as rotas para visualizar", rotas_disponiveis, default=rotas_disponiveis)

# Mostrar mapas
if st.checkbox("Mostrar mapas das rotas"):
    for i in range(len(res["routes"])):
        if f"Rota {i}" in rotas_selecionadas:
            plot_routes_by_vehicle({
                **res,
                "routes": [res["routes"][i]],
                "cargo": [res["cargo"][i]],
                "assign": [a for a in res["assign"] if a[0] == i],
                "dists": [res["dists"][i]]
            }, figsize=(7,7))
