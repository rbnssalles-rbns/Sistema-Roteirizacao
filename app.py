...
def daily_summary(result):
    ...
    # Sinalização de anomalias (rotas < min_clients) e clientes não roteados
...
def rebalance_routes(res, min_clients=10, max_clients=25):
    ...
    - Retorna também listas de anomalias e clientes originalmente não agrupáveis
    """

    rotas_final = []
    cargo_final = []
    dispersos_rows = []      # clientes de rotas pequenas para reagrupamento
    anomalies_labels = []    # texto de rotas anômalas
    unrouted_ids = []        # ids de clientes que precisaram ir para rotas < min_clients

    # 1) Processa rotas atuais
    for i, r in enumerate(res["routes"]):
        stops = len(r) - 2
        if stops > max_clients:
            # Divide em blocos de até max_clients
    ...
    # 2) Tenta reotimizar clientes dispersos
    if dispersos_rows:
        ...
        # Primeiro, adiciona só rotas com >= min_clients
        for nr, nc in zip(novas_rotas, novas_cargas):
            if len(nr) - 2 >= min_clients:
                rotas_final.append(nr)
                cargo_final.append(nc)

        # Depois, verifica se sobraram clientes que não entraram em rotas >= min_clients
        roteados = set()
        for r in rotas_final:
            for node in r[1:-1]:
                roteados.add(node)
        # Todos os nodes possíveis para pedidos_rebalancear são 1..N naquela submatriz
        # Vamos mapear pelos índices do dataframe original do dia
        # Como os nodes são baseados na posição no day_orders (1-based),
        # já coletamos os rows originais, então identificamos quem não entrou:
        restantes_rows = []
        for row in dispersos_rows:
            node_global = res["orders"].index[res["orders"]["client_id"] == row["client_id"]].tolist()
            # Garantia simples: se não achou por client_id, ignora; senão verifica node_global+1
            if node_global:
                node_idx = node_global[0] + 1
                if node_idx not in roteados:
                    restantes_rows.append(row)

        # Se ainda há restantes, força rotas anômalas para garantir entrega
        if restantes_rows:
            # Estratégia simples: agrupar em blocos do próprio algoritmo; se vier < min, aceita e marca
            pedidos_restantes = pd.DataFrame(restantes_rows)
            nodes_rest, dist_km_rest, time_h_rest = build_costs_for_day(pedidos_restantes)
            rotas_rest, cargas_rest = clarke_wright(pedidos_restantes, dist_km_rest, time_h_rest, FLEET)
            for nr, nc in zip(rotas_rest, cargas_rest):
                stops_nr = len(nr) - 2
                if stops_nr < min_clients:
                    # Marca anomalia e registra ids
                    ids = []
                    for node in nr[1:-1]:
                        # node refere-se ao índice em pedidos_restantes; mapeia para client_id
                        cid = pedidos_restantes.iloc[node - 1]["client_id"]
                        ids.append(str(cid))
                        unrouted_ids.append(cid)
                    anomalies_labels.append(f"Rota anômala com {stops_nr} clientes: IDs {', '.join(ids)}")
                rotas_final.append(nr)
                cargo_final.append(nc)

    # 3) Reatribui veículos
    assignments_final, dists_final = assign_vehicles_to_routes(rotas_final, cargo_final, FLEET, res["dist_km"])

    # 4) Recalcula violações de janelas (usando matriz de tempo/distância do dia completo)
    violations = []
    ...
    for idx_r, r in enumerate(rotas_final):
        ...
        for node in r[1:-1]:
            prev = r[r.index(node) - 1]
            travel_h = res["nodes"] and res["dist_km"][prev, node] or 0
            t += dt.timedelta(hours=travel_h)
...
if st.button("🔄 Reprocessar rotas", key="btn_reprocessar"):
    ...
    # Mapas pós-reprocessamento (não recria multiselect)
