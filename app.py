...
if st.button("Simular semana completa com extras", key="btn_semana_completa_extras"):
    ...
    for d in DATES:
        ...
        try:
            veiculos_usados = len({
                item[1] for item in assigns
                if isinstance(item, (list, tuple)) and len(item) >= 2 and item[1] is not None
            })
        except Exception as e:
            st.error(f"Erro ao calcular veículos usados: {e}")

        resumo_semana.append({
            "Data": d.date(),
            "Rotas": len(res_dia.get("routes", [])),
            "Veículos": veiculos_usados,
            "Distância total (km)": sum(r[2] for r in res_dia.get("routes", [])),
...
if "resumo_semana" in st.session_state and "dias_simulados" in st.session_state:
    for d, res_dia in st.session_state["dias_simulados"]:
        ...
        if res_dia.get("orders", None) is not None and not res_dia["orders"].empty:
