...
def rebalance_routes(res, min_clients=10, max_clients=25):
    ...
    - Agrupa rotas pequenas (< min_clients) para formar rotas viáveis
    - Remove rotas com menos de min_clients no final
    - Reatribui veículos e recalcula métricas
    """

    rotas_final = []
    cargo_final = []

    # 1. Processa cada rota existente
    ...
    # 2. Agrupa clientes de rotas pequenas
    clientes_pequenos = []
    for i, r in enumerate(res["routes"]):
        stops = len(r) - 2
        if stops < min_clients:
            for node in r[1:-1]:
                clientes_pequenos.append(res["orders"].iloc[node-1])

    if clientes_pequenos:
        pedidos_rebalancear = pd.DataFrame(clientes_pequenos)
        ...
        rotas_final.extend(novas_rotas)
        cargo_final.extend(novas_cargas)

    # 3. Remove rotas com menos de min_clients
    rotas_filtradas = []
    cargo_filtrada = []
    for r, c in zip(rotas_final, cargo_final):
        if len(r) - 2 >= min_clients:
            rotas_filtradas.append(r)
            cargo_filtrada.append(c)

    # 4. Reatribui veículos
    assignments_final, dists_final = assign_vehicles_to_routes(rotas_filtradas, cargo_filtrada, FLEET, res["dist_km"])

    # 5. Recalcula violações de janelas
    violations = []
    start_dt = dt.datetime.combine(res["date"].date(), WORK_START)
    for idx_r, r in enumerate(rotas_filtradas):
        t = start_dt
        for node in r[1:-1]:
            prev = r[r.index(node) - 1]
    ...
    return {
        "date": res["date"],
        "routes": rotas_filtradas,
        "cargo": cargo_filtrada,
...
# Filtro de rotas (multiselect único, sem duplicação de key)
...
if st.button("🔄 Reprocessar rotas", key="btn_reprocessar"):
    # 1) Recalcula o dia cru
    res_raw = simulate_day(dia_selecionado, DAILY)
    # 2) Rebalanceia para 10–25 clientes por rota
    ...
    # Não recria multiselect aqui — já existe acima

    if st.checkbox("Mostrar mapas das rotas", key="mostrar_mapas_checkbox_reprocessar"):
        for i in range(len(res["routes"])):
            if f"Rota {i}" in st.session_state.rotas_selecionadas:
                with st.container():
                    plot_routes_by_vehicle(res, i, figsize=(7, 7))
