...
#função build_folium_map
import streamlit as st
import pandas as pd
import folium
from streamlit_folium import st_folium

def build_folium_map(res_dia):
    orders = res_dia.get("orders", None)

    if orders is not None and not orders.empty:
        lat0, lon0 = orders.iloc[0]["lat"], orders.iloc[0]["lon"]
        mapa = folium.Map(location=[lat0, lon0], zoom_start=11)
    else:
        mapa = folium.Map(location=[-3.717, -38.543], zoom_start=11)

    cores = ["blue", "green", "purple", "orange", "cadetblue", "darkred"]

    for idx, rota in enumerate(res_dia.get("routes", [])):
        cor = cores[idx % len(cores)]
        violacoes_rota = [v for v in res_dia.get("violations", []) if v.get("route") == idx]
        if violacoes_rota:
            cor = "red"

        pontos = []
        for node in rota:
            if node > 0 and orders is not None and not orders.empty:
                try:
                    ord_row = orders.iloc[node - 1]
                    lat, lon = ord_row["lat"], ord_row["lon"]
                    pontos.append((lat, lon))
                    folium.Marker(
                        location=[lat, lon],
                        popup=f"Cliente {ord_row['client_id']}",
        ...
        if pontos:
            folium.PolyLine(pontos, color=cor, weight=4, opacity=0.8).add_to(mapa)

    return mapa

# Parte 7 — Simulação semanal com extras (upgrade)
# Função reutilizável para simular a semana completa
def simular_semana_completa():
    resumo_semana = []
    dias_simulados = []
    ...
    for d in DATES:
        ...
        for pedido in st.session_state.pedidos_extras:
            if pd.to_datetime(pedido["date"]) == d:
                df_mod = add_last_minute_order(
                    pedido["date"], pedido["client_id"], pedido["sku_qty"], df_mod
                )

        res_dia = simulate_day(d, df_mod)

        # ✅ Contagem correta de veículos via objetos VehicleType
        assigns = res_dia.get("assign", [])
        veiculos_usados = len({
            v.name for (_, v) in assigns
            if isinstance(v, VehicleType) and v is not None
        })

        # Proteção para distância total
        try:
            distancia_total = sum(
                r[2] for r in res_dia.get("routes", [])
        ...
        resumo_semana.append({
            "Data": d.date(),
            "Rotas": len(res_dia.get("routes", [])),
            "Veículos": veiculos_usados,
            "Distância total (km)": distancia_total,
            "Violações": len(res_dia.get("violations", [])),
        })

        dias_simulados.append((d, res_dia))

    st.session_state["resumo_semana"] = resumo_semana
    st.session_state["dias_simulados"] = dias_simulados

# Parte 7 — Interface principal
...
if st.button("Simular semana completa com extras", key="btn_semana_completa_extras"):
    simular_semana_completa()

# Botões lado a lado
col1, col2 = st.columns(2)

with col1:
    if st.button("🔄 Reexecutar simulação", key="btn_reexecutar_semana"):
        for k in ["resumo_semana", "dias_simulados"]:
            st.session_state.pop(k, None)
        simular_semana_completa()

with col2:
    if st.button("🧹 Limpar resultado", key="btn_limpar_semana"):
        for k in ["resumo_semana", "dias_simulados"]:
            st.session_state.pop(k, None)
        st.success("Resultado da simulação foi limpo.")

# Exibe resultados se já foram simulados
if "resumo_semana" in st.session_state and "dias_simulados" in st.session_state:
    for d, res_dia in st.session_state["dias_simulados"]:
        st.markdown(f"### 📅 {d.date()}")
        daily_summary(res_dia)

        if len(res_dia.get("violations", [])) > 15:
            st.warning("⚠️ Muitas janelas violadas neste dia.")

        orders = res_dia.get("orders", None)
        if orders is not None and not getattr(orders, "empty", False):
            mapa = build_folium_map(res_dia)
            st_folium(mapa, width=700, height=500)
        else:
            st.info("ℹ️ Nenhum pedido encontrado para gerar o mapa.")

    df_resumo = pd.DataFrame(st.session_state["resumo_semana"])
