...
# =========================
# Importações e estado
# =========================

import folium, json
from streamlit_folium import st_folium

# Histórico de extras por data (dict: date -> list de pedidos)
if "pedidos_extras" not in st.session_state:
    st.session_state.pedidos_extras = {}
if "res_next" not in st.session_state:
    st.session_state.res_next = None
if "res_mod" not in st.session_state:
    st.session_state.res_mod = None
if "res_semana" not in st.session_state:
    st.session_state.res_semana = {}

# Inicialização segura dos campos de entrada
if "cliente_extra" not in st.session_state:
    st.session_state["cliente_extra"] = "Selecione..."
for sku in CATALOG["sku"].unique():
    if f"qtd_{sku}" not in st.session_state:
        st.session_state[f"qtd_{sku}"] = 0
...
def _map_center_from_orders(res):
    if "orders" in res and not res["orders"].empty and {"lat","lon"}.issubset(res["orders"].columns):
        lat_mean = res["orders"]["lat"].mean()
        lon_mean = res["orders"]["lon"].mean()
        return (float(lat_mean), float(lon_mean))
    return (-3.732, -38.526)  # Fortaleza (fallback)

def plot_routes_map(res, depot_coords=None, zoom_start=11):
    center = depot_coords if depot_coords is not None else _map_center_from_orders(res)
    m = folium.Map(location=center, zoom_start=zoom_start)
    colors = ["blue","green","purple","orange","darkred","cadetblue","darkgreen","darkblue"]

    if depot_coords is not None:
        folium.Marker([depot_coords[0], depot_coords[1]], popup="Depósito",
                      icon=folium.Icon(color="red", icon="home")).add_to(m)

    for i, route in enumerate(res["routes"]):
        coords = []
        for node in route:
            if node == 0 and depot_coords is not None:
                coords.append((depot_coords[0], depot_coords[1]))
            else:
                order = res["orders"].iloc[node-1]
                lat, lon = float(order["lat"]), float(order["lon"])
                coords.append((lat, lon))
                folium.Marker([lat, lon],
                              popup=f"Cliente {order['client_id']} (Rota {i})",
                              icon=folium.Icon(color=colors[i % len(colors)], icon="user")).add_to(m)
        if len(coords) >= 2:
            folium.PolyLine(coords, color=colors[i % len(colors)], weight=3, opacity=0.9).add_to(m)

    st_folium(m, width=720, height=520)

def validate_min_clients(res, minimo=20, title_prefix=""):
    st.subheader(f"✅ Validação de mínimo de clientes {title_prefix}".strip())
    for i, route in enumerate(res["routes"]):
        num_clientes = sum(1 for n in route if n != 0)
        vol, wt, _ = res["cargo"][i]
        assign = [v for k, v in res["assign"] if k == i][0]
        custo_fixo = getattr(assign, "fixed_cost", 0)
        custo_por_cliente = (custo_fixo / max(num_clientes, 1)) if custo_fixo else 0

        if num_clientes < minimo:
            st.error(
                f"⚠️ Exceção: Rota {i} tem {num_clientes} clientes (< {minimo}). "
                f"Veículo {assign.name} subutilizado. "
                f"Custo estimado por cliente: R$ {custo_por_cliente:.2f}"
            )
        else:
            st.success(f"Rota {i} atende ao mínimo de {minimo} clientes ({num_clientes}).")

# =====================================
# Checkbox global
# =====================================

mostrar_mapa = st.checkbox("Mostrar mapas das rotas", value=True)
...
st.subheader("🔎 Análise de capacidade do dia selecionado")
res_dia = simulate_day(data_corrente, DAILY)

for i, _ in enumerate(res_dia["routes"]):
    vol, wt, _ = res_dia["cargo"][i]
    assign = [v for k, v in res_dia["assign"] if k == i][0]
    ocup_vol = vol / assign.vol_m3 * 100 if assign.vol_m3 else 0
    ocup_wt = wt / assign.weight_kg * 100 if assign.weight_kg else 0
    st.write(f"Rota {i} – Veículo {assign.name}: Ocupação {ocup_vol:.0f}% vol / {ocup_wt:.0f}% peso")
    if ocup_vol > 90 or ocup_wt > 90:
        st.warning("⚠️ Capacidade quase cheia — cuidado ao adicionar pedidos extras.")
    else:
        st.success("✅ Há margem para pedidos extras.")

# =====================================
# Interface de pedidos extras + três botões lado a lado
# =====================================
...
# Selectbox com opção placeholder para limpar
clients_opts = ["Selecione..."] + sorted(CLIENTS["client_id"].unique().tolist())
cliente_extra = st.selectbox(
    "Cliente",
    clients_opts,
    key="cliente_extra"
)

# Inputs dinâmicos de SKUs
sku_inputs = {}
for sku in CATALOG["sku"].unique():
...
# Três botões lado a lado (sempre visíveis)
col1, col2, col3 = st.columns(3)

# Botão 1: Adicionar pedido extra e replanejar
with col1:
    if st.button("Adicionar pedido extra e replanejar", key="btn_pedido_extra"):
        # Validações básicas
        if cliente_extra == "Selecione...":
            st.warning("Selecione um cliente antes de adicionar o pedido extra.")
        elif len(sku_inputs) == 0:
            st.warning("Informe ao menos 1 SKU com quantidade > 0.")
        else:
            data_key = pd.to_datetime(data_extra).date()
            if data_key not in st.session_state.pedidos_extras:
                st.session_state.pedidos_extras[data_key] = []
            novo_pedido = {"date": pd.to_datetime(data_extra), "client_id": cliente_extra, "sku_qty": sku_inputs}
            st.session_state.pedidos_extras[data_key].append(novo_pedido)

            # Replaneja apenas com extras daquele dia
            df_mod = DAILY.copy()
            for pedido in st.session_state.pedidos_extras[data_key]:
            ...
            st.session_state.res_mod = res_mod

            # Tenta incluir sem reotimizar se cliente já está na rota e cabe
            ...
            # Validação mínima (após pedido extra)
            validate_min_clients(res_mod, minimo=20, title_prefix="(após pedido extra)")

            # Exportação CSV
            st.subheader("💾 Exportar cenário modificado")
            df_export = df_mod.copy()
            if "sku_qty" in df_export.columns:
                df_export["sku_qty"] = df_export["sku_qty"].apply(
                    lambda d: json.dumps(d, ensure_ascii=False) if isinstance(d, dict) else str(d)
                )
            csv_bytes = df_export.to_csv(index=False).encode("utf-8")
            st.download_button(
                label="Baixar CSV dos pedidos (com extras)",
                data=csv_bytes,
                file_name=f"pedidos_modificados_{pd.to_datetime(data_extra).date()}.csv",
                mime="text/csv"
            )

            # Limpa campos com proteção (mantém botões visíveis)
            st.session_state.update({
                "cliente_extra": "Selecione...",
                **{f"qtd_{sku}": 0 for sku in CATALOG["sku"].unique()}
            })

# Botão 2: Inserir novo pedido (apenas limpa e libera os campos)
with col2:
    if st.button("Inserir novo pedido", key="btn_novo_pedido"):
        st.session_state.update({
            "cliente_extra": "Selecione...",
            **{f"qtd_{sku}": 0 for sku in CATALOG["sku"].unique()}
        })
        st.info("Campos liberados para inserir novo pedido.")

# Botão 3: Planejar dia seguinte (D+1)
with col3:
    if st.button("Planejar dia seguinte (D+1)", key="btn_plan_next"):
        ...
        if idx >= len(DATES) - 1:
            ...
            st.session_state.res_next = res_next
            st.markdown(f"### 📅 {next_date.date()}")
            daily_summary(res_next)
            for i in range(len(res_next["routes"])):
                plot_routes_by_vehicle(res_next, i, figsize=(7,7))
            validate_min_clients(res_next, minimo=20, title_prefix="(D+1)")

# Mapas condicionados
if mostrar_mapa and st.session_state.res_next:
    DEPOT_COORDS = st.session_state.get("depot_coords", None)
    st.subheader("🗺️ Mapa interativo das rotas (D+1)")
    plot_routes_map(st.session_state.res_next, depot_coords=DEPOT_COORDS)

if mostrar_mapa and st.session_state.res_mod:
    DEPOT_COORDS = st.session_state.get("depot_coords", None)
    st.subheader("🗺️ Mapa interativo das rotas (após pedido extra)")
    plot_routes_map(st.session_state.res_mod, depot_coords=DEPOT_COORDS)

# =====================================
# Histórico de pedidos extras por dia
# =====================================

st.subheader("📋 Pedidos extras acumulados por dia")
if st.session_state.pedidos_extras:
    datas_hist = sorted(st.session_state.pedidos_extras.keys())
    data_hist_sel = st.selectbox(
        "Selecione a data para ver o histórico de extras",
        datas_hist,
        format_func=lambda d: pd.to_datetime(d).strftime("%Y-%m-%d"),
        key="data_hist_sel"
    ) if datas_hist else None

    if data_hist_sel and st.session_state.pedidos_extras.get(data_hist_sel):
        df_extras = pd.DataFrame(st.session_state.pedidos_extras[data_hist_sel])
        df_extras_view = df_extras.copy()
        df_extras_view["sku_qty"] = df_extras_view["sku_qty"].apply(lambda d: json.dumps(d, ensure_ascii=False))
        st.dataframe(df_extras_view, use_container_width=True)
    else:
        st.info("Sem pedidos extras registrados para a data selecionada.")

# =====================================
# Planejamento rolling automatizado (semana)
# =====================================
...
if st.button("Simular semana completa", key="btn_semana"):
    ...
    for d in DATES:
        df_mod = DAILY.copy()
        data_key = pd.to_datetime(d).date()
        if data_key in st.session_state.pedidos_extras:
            for pedido in st.session_state.pedidos_extras[data_key]:
                df_mod = add_last_minute_order(pedido["date"], pedido["client_id"], pedido["sku_qty"], df_mod)

        res_dia = simulate_day(d, df_mod)
        st.session_state.res_semana[d] = res_dia
        st.markdown(f"### 📅 {pd.to_datetime(d).date()}")
        daily_summary(res_dia)
        for i in range(len(res_dia["routes"])):
            plot_routes_by_vehicle(res_dia, i, figsize=(6,6))
        validate_min_clients(res_dia, minimo=20, title_prefix=f"({pd.to_datetime(d).date()})")
        if mostrar_mapa:
            plot_routes_map(res_dia, depot_coords=DEPOT_COORDS, zoom_start=10)
