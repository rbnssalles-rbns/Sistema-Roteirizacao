...
# =========================
# Bloco 6 — Execução por dia e relatório
# =========================
...
# -------------------------
# Parte B — Interface Streamlit (consolidada)
# -------------------------
...
if st.button("🔄 Reprocessar rotas", key="btn_reprocessar_principal"):
    ...
    rotas_disponiveis = [f"Rota {i}" for i in range(len(res["routes"]))]  # corrigido
    st.session_state.rotas_selecionadas = [
...
# Planejamento D+1 (ajustado para considerar estado e extras)
...
if st.button("Planejar dia seguinte (D+1)", key="btn_plan_next_rolling"):
    ...
    if idx >= len(DATES) - 1:
        ...
        # Base: DAILY + pedidos extras dessa data (se houver)
        df_base = DAILY.copy()
        ...
        # Simula com base atualizada (respeitando estado e extras)
...
if st.button("Simular semana completa com extras", key="btn_semana_completa_extras"):
    for d in DATES:
        df_mod = DAILY.copy()
        # aplica pedidos extras dessa data, se existirem
        for pedido in st.session_state.pedidos_extras:
