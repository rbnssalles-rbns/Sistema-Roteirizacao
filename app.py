...
if st.button("Simular semana completa com extras", key="btn_semana_completa_extras"):
    ...
    for d in DATES:
        ...
        # ✅ Contagem correta de veículos usados via routes_info
        routes_info = res_dia.get("routes_info", [])
        veiculos_usados = len({
            r.get("vehicle") for r in routes_info
            if isinstance(r, dict) and r.get("vehicle")
        })
